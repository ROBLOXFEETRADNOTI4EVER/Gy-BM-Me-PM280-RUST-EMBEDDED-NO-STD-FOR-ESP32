...
impl bmp_uart {
    ...
    async fn read_temperature(&mut self) -> Option<f32> {
...
async fn main(spawner: Spawner) {
    ...
    let bmp_i2c: I2c<'static, esp_hal::Async> = I2c::new(
        peripherals.I2C0,
        Config::default().with_frequency(Rate::from_khz(150)),
    )
    .unwrap()
    ...
    // let _ = spawner;
    info!(
        "checking if the chips id is the same as what is being read:{:#?}",
        bmp280.check_chip().await
    );
    ...
    loop {
        ...
        Timer::after(Duration::from_millis(300)).await;
        match bmp280.read_altitude(1021.0).await {
            // 1013.25 if checking from pecs
            Some(data) => {
